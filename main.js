...
function takeCommand(message){
    if(message.includes('hey') || message.includes('hello')){
        ...
        const query = message.replace('on youtube', '').trim(); // Removes 'on youtube' from the command
        window.open(`https://www.youtube.com/results?search_query=${query.replace(" ", "+")}`, "_blank");
        const finalText = `Here are the results for ${query} on YouTube`;
        speak(finalText);
    }
